...
if (filesToUse !== defaultFilesToUse) {
  console.log(`Found custom rule to identify files to use: ${filesToUse}`);
} else {
  console.log('Using default rule to identify files');
}
...
(async function main(): Promise<void> {
  ...
  try {
    ...
    const globalResults = results.reduce(
      (globalAcc, result) => {
        globalAcc.errorCount += result.errorCount; // eslint-disable-line no-param-reassign
        globalAcc.warningCount += result.warningCount; // eslint-disable-line no-param-reassign
        return globalAcc;
      },
